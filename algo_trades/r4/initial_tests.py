...
class Macarons(ProductTrader):
    def __init__(self, state: TradingState, traderData: Dict, result: Dict[Symbol, List[Order]], ppw: int):
        ...
        if (self.curr_pos == 0):
            ...
            result[self.name] = orders
            self.can_convert = 0
            self.trader_data['short_price'] = self.max_bid + 1
            return
        ...
        self.import_loss = self.conv_observations.askPrice + self.conv_observations.transportFees + self.conv_observations.importTariff
        self.export_gain = self.conv_observations.bidPrice + self.conv_observations.transportFees - self.conv_observations.exportTariff 
    ...
    def convert(self, result:Dict[Symbol, List[Order]])-> int:
        ...
        island_sell = self.trader_data.get('short_price', 0)

   
        
        if self.import_loss < island_sell:
            logger.print("importing!")
            orders.append(Order(self.name, self.max_bid + 1, self.curr_pos))
            result[self.name] = orders
            self.traded_once = 1
            return -self.curr_pos
        
        elif self.trader_data['short_price'] >= self.min_ask - 1:
            orders.append(Order(self.name,self.min_ask - 1, -self.curr_pos))
            orders.append(Order(self.name, self.max_bid + 1, -20))
            result[self.name] = orders
            self.can_convert = 0
            self.trader_data['short_price'] = self.max_bid + 1
            return 0
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        mm = Macarons(state, traderData, result, -1)
        conversions = mm.convert(result)

        if hasattr(state.observations, "conversionObservations"):
            conv_observations = state.observations.conversionObservations.get("MAGNIFICENT_MACARONS", None)
        else:
            conv_observations = None


        if conv_observations is None:
            logger.print("Observation not available!")
        else:
            logger.print(conv_observations.__str__)
